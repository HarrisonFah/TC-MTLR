# TC-MTLR
Code for reproducing results from the paper "Temporally-Consistent Multi-Task Logistic Regression"

## Run code (temp): python main.py --config configs/config_pbc.yaml

## Installing Packages

## Recreating Data Files
While we do not provide the data used in the original experiments, we do provide instructions and code for recreating the files for each dataset.

### PBC

### AIDS

### RW

### LastFM

### NASA

### BigRW
