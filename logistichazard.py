...
class LogisticHazardSA(object):
	...
	def init_networks(self, time_bins):
		...
		self.model = LogisticHazard(self.network, optimizer=self.optimizer, device=device, duration_index=self.time_bins)

	def train(self, train_gen):
		state, _, _, _, times, censors = train_gen.get_all_data()
		x = state
		times = times.reshape((-1,1))
		censors = censors.reshape((-1,1))
