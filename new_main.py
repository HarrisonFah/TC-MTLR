...
from utils import median_time_bins, get_train_val_test_mtlr, get_data, train_val_test_split
...
if __name__ == '__main__':  
    ...
    for type_agent in ['SA_Init_Cox', 'SA_Landmark_Cox', 'LambdaSA_Cox', 'DeepLambdaSA_Cox', 'TC_MTLR']:
        if type_agent == 'SA_Init_Cox':
            ...
            config['calculate_tgt_and_mask'] = True
        ...
        # dataset info
        seqs, ts, cs, target, h_ws, mask, rs, seqs_ts = get_data(config['dataset_kwargs']['data_path'],
                                                    config['landmark'],
                                                    config['calculate_tgt_and_mask'])
        print('main cs.shape:', cs.shape)
        data_arrays = train_val_test_split(seqs, target, h_ws, mask, ts, cs, rs, seqs_ts, seed, val_size=0.15, test_size=0.2)
        seqs = seqs.astype(np.float32)
        config['input_dim'] = seqs.shape[2]

        mtlr_gens = get_train_val_test_mtlr(data_arrays, config['batch_size'])

        time_bins = median_time_bins(data_arrays, config['dataset_kwargs']['horizon'])
